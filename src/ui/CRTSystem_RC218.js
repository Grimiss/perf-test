import { GameRuntime } from "./GameRuntime.js?v=rc215";
import { getPerformanceMode, getPerformanceProfile, togglePerformanceMode } from "../perf/PerformanceProfile.js?v=rc215";
...
export class CRTSystem {
  ...
  bind() {
    ...
    this.root.addEventListener('click', (e) => {
      ...
      if (action === 'select-ss') this.controller.selectSoundscape(el.dataset.id);
        ...
        el.textContent=`PERF: ${mode.toUpperCase()}`;
        el.setAttribute('aria-pressed',String(mode==='smooth'));
    ...
    this.root.addEventListener('input', (e) => {
      ...
      else if (kind === 'fxmod-flight') this.controller.setFxModFlight(value);
  ...
  render(state, force=false) {
    ...
    if(this.currentScreen==='tv' && screen!=='tv'){
      const tvVideo=this.root.querySelector('[data-tv-soundscape-video]');
  ...
  patchScreen(state, screen=this.currentScreen) {
    ...
    if (screen === 'byd') {
      ...
      setText('.byd-screen [data-current-soundscape]', state.soundscape.name);
      state.tracks.forEach((track,i)=>setText(`.byd-screen .byd2-channel[data-dev-key="ch${i+1}"] .byd2-channel-name`, track.name||''));
      return;
    }

    if (screen === 'control-centre') {
      setText('.cc3-screen [data-current-soundscape]', state.soundscape.name);
      state.tracks.forEach((track,i)=>setText(`.cc3-screen .cc3-channel[data-dev-key="ch${i+1}"] .cc3-channel-name`, track.name||''));
      state.tracks.forEach((track, i) => {
      ...
      const flightInput=this.root.querySelector('input[data-kind="fxmod-flight"]');
      if(flightInput){ if(document.activeElement!==flightInput) flightInput.value=String(Math.round(state.fxMod.flightMs||4000)); flightInput.disabled=Boolean(state.fxMod.infinite); }
    ...
    if (screen === 'tv') {
      const mounted=this.root.querySelector('[data-tv-ss-id]');
      if(!mounted || mounted.dataset.tvSsId!==state.soundscape.id){
        this.clearDomCache();
        this.renderTV(state);
      }
      setText('.missing-note', `TV MODE · ${state.frs.toUpperCase()} ${state.frs==='focus'?'100':state.frs==='relax'?'75':'50'}%`);
      this.syncTVVideo(state);
    }
  }
...
Choose your destination:  "><div class="spectrum-typed-line"><span class="typed-copy"></span><span class="spectrum-type-cursor">L</span></div></div>
  ...
  renderTV(state) {
    ...
    this.root.innerHTML=`<div class="crt-screen missing-screen tv-video-screen" data-tv-ss-id="${state.soundscape.id}"><div class="missing-big">MISSING ASSET</div><div class="missing-file">${filename}</div><div class="missing-note">TV MODE · ${state.frs.toUpperCase()} ${state.frs==='focus'?'100':state.frs==='relax'?'75':'50'}%</div></div>`;
  }
